...
int task_init (task_t *task, const char * name, int flag, uint32_t entry, uint32_t esp) {
    ...
    kernel_memset(task->file_tab,0, sizeof(task->file_tab)); // 清零
...
int sys_fork (void) {
    ...
    for (int i = 0; i < TASK_FILE_SIZE; ++i) {
        file_t *file=parent_task->file_tab[i];
        if(file){
            file->ref++;
            child_task->file_tab[i]=file;
        }
    }
...
file_t *task_file(int fd){
    task_t *task= task_current();
    return task->file_tab[fd];
}

int task_alloc_fd(file_t *file){ // 分配文件并返回索引
    task_t *task= task_current();
    for (int i = 0; i < TASK_FILE_SIZE; ++i) {
        if(task->file_tab[i]==0){
            task->file_tab[i]=file;
            return i;
        }
    }
    return -1;
...
int sys_dup(int fd){// 复制指定文件到新空间
    file_t *file =task_file(fd);
    file->ref++;
    return task_alloc_fd(file);
}

void sys_exit(int state){
    task_t *task=task_current();
    for (int fd = 0; fd < TASK_FILE_SIZE; ++fd) {// 清理文件资源
        if(task->file_tab[fd]){
            sys_close(fd);
            task->file_tab[fd]=0;
        }
    ...
    for (int i = 0; i < TASK_NR; ++i) {
        ...
        if(temp->parent==task){ // 当前进程即将销毁，若其有子进程需要转交给其他进程
            temp->parent=&task_manager.first_task;
        }
    }
...
int sys_wait(int *code){
    task_t *task=task_current();
    for (;;){
        for (int i = 0; i < TASK_NR; ++i) {
            task_t *temp=task_table+i;
            if(temp->parent!=task||temp->state!=TASK_ZOMBIE){
                continue;
            }
            ...
            *code=temp->code;
            // 释放栈空间与任务结构
            memory_free_page(temp->tss.esp0-MEM_PAGE_SIZE);
            kernel_memset(temp,0,sizeof(task_t));
            return pid;
        }
        // 节约资源，等待子进程有需要时再唤醒
