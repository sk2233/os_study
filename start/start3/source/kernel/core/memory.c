...
// 分配堆内存，出错返回-1 0查看当前堆空间的结束地址 正数分配一定的字节空间返回之前的结束地址
char *sys_sbrk(ptrdiff_t incr){
    ASSERT(incr>=0);
    task_t *task= task_current();
    char *res = (char *)task->heap_end;
    if(incr==0){// 查看结束地址
        return res;
    }

    uint32_t old = task->heap_end;
    task->heap_end+=incr;
    if((old-1)/MEM_PAGE_SIZE==(task->heap_end-1)/MEM_PAGE_SIZE){
        return res; //还在一个页内不需要分配内存块
    }
    old= up2(old,MEM_PAGE_SIZE);// 取整到最近
    memory_alloc_page_for(old,task->heap_end-old,PTE_P|PTE_W|PTE_U);// 分配内存页  size内部会向上取整这里不用管
    return res;
}