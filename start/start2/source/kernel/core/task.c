...
void load_phdr(int file, Elf32_Phdr *phdr, uint32_t page_dir) {
    memory_alloc_for_page_dir(page_dir,phdr->p_vaddr,phdr->p_memsz,PTE_P|PTE_U|PTE_W);
    sys_seek(file,(int )phdr->p_offset,0);

    uint32_t vaddr = phdr->p_vaddr;
    uint32_t size = phdr->p_filesz;
    while (size>0){
        // 要保证在虚拟地址paddr中是连续的，现在地址使用的不是paddr的虚拟地址，只能找其物理地址一页页拷贝
        // 默认分页低于2g的空间都是与物理地址一一映射的，可以认为是物理地址
        uint32_t read_size = size>MEM_PAGE_SIZE?MEM_PAGE_SIZE:size;
        uint32_t paddr = memory_get_paddr(page_dir,vaddr);
        sys_read(file,(char *)paddr,(int )read_size);
        vaddr+=read_size;
        size-=read_size;
    }
}

uint32_t load_elf_file(task_t *task, char *name, uint32_t page_dir){ // 加载指定文件名称 以对应的任务与页表
    Elf32_Ehdr ehdr;
    Elf32_Phdr phdr;
    int file=sys_open(name,0);

    sys_read(file,(char *)&ehdr, sizeof(ehdr));
    if(ehdr.e_ident[0]!=0x7F||ehdr.e_ident[1]!='E'||ehdr.e_ident[2]!='L'||ehdr.e_ident[3]!='F'){
        PANIC("err elf header");
    }

    int phoff =(int )ehdr.e_phoff;
    for (int i = 0; i < ehdr.e_phnum; ++i) {
        sys_seek(file,phoff,0);
        sys_read(file,(char *)&phdr, sizeof(phdr));
        if(phdr.p_type!=1){
            log_printf("not exe type skip");
            continue;
        }
...
void copy_args(char *desc, uint32_t page_dir, int argc, char **strs) {

}

int sys_execve (char *name, char **args, char **env) {
    task_t *task= task_current();
    kernel_strcpy(task->name,get_file_name(name)); // 切换任务名称为文件名称
    // 切换页表
    uint32_t new_uvm=memory_create_uvm();
    uint32_t entry = load_elf_file(task,name,new_uvm);
    // 分配栈空间
    memory_alloc_for_page_dir(new_uvm,MEM_TASK_STACK_TOP-MEM_TASK_STACK_SIZE,MEM_TASK_STACK_SIZE,
                              PTE_P|PTE_U|PTE_W);
    int argc = strings_count(args);
    copy_args((char *)(MEM_TASK_STACK_TOP-MEM_TASK_ARG_SIZE),new_uvm,argc,args);
    syscall_frame_t *frame=(syscall_frame_t*)(task->tss.esp0- sizeof(syscall_frame_t));
    frame->eip=(int)entry; // 修改返回地址
    // 修改栈空间,因为汇编中最终返回有弹栈操作要预留相关位置 还要为参数压栈预留位置
    frame->esp=MEM_TASK_STACK_TOP-MEM_TASK_ARG_SIZE-4*SYSCALL_PARAM_COUNT;
    frame->eflags=EFLAGS_IF|EFLAGS_DEFAULT;// 改回默认值 准备进入新函数
    frame->eax=frame->ebx=frame->ecx=frame->edx=0; // 清空之前的变量
    memory_destroy_uvm(task->tss.cr3); // 销毁之前的页表
    task->tss.cr3=new_uvm;
    // 创建新的页表并写入 cr3 使立即生效
    mmu_set_page_dir(new_uvm);
    return -1;
}
